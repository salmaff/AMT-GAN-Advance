...
#Membuat sebuah layer yang terdiri dari beberapa blok yang sama.
#Input: Gambar wajah tanpa makeup.
#Jaringan Neural:
# Lapisan pertama: Mengidentifikasi struktur wajah (dimana bibir, mata, kulit, dll).
# Lapisan kedua: Menentukan dimana harus menerapkan makeup.
# Lapisan ketiga: Menerapkan perubahan kecil (misalnya, warna pada bibir).
# Lapisan terakhir: Menggabungkan semua elemen makeup dengan gambar wajah asli, menghasilkan wajah dengan makeup.
...
#Menggunakan Kaiming Normal Initialization, yang dirancang untuk lapisan non-linearitas ReLU,
#dengan memodifikasi bobot untuk lapisan konvolusi dan linear.
...
####################################################### Blocks #########################################################
#meningkatkan performa model dalam menangani transformasi wajah seperti makeup. 
#Ia menggunakan Conv2D dengan normalisasi instan (InstanceNorm) dan aktivasi ReLU untuk memproses fitur wajah.
...
#Mendapatkan matriks gamma dan beta yang digunakan untuk menyesuaikan gaya makeup pada wajah.
# Matriks ini akan dihasilkan melalui layer konvolusi, dan digunakan dalam proses transfer makeup,
# menentukan bagaimana fitur wajah dimodifikasi untuk mengaplikasikan makeup.
...
#Blok non-lokal yang berfungsi untuk menangkap dependensi jarak jauh antara piksel gambar. 
#Ini membantu menangkap konteks yang lebih luas dari gambar yang diproses.
#Digunakan dalam bottleneck network untuk memperhatikan area penting dalam wajah saat menerapkan makeup.
...
#Misalkan kita punya input gambar dengan dimensi (64 channel, 256x256 piksel).
# - Conv1 akan menghasilkan fitur-fitur baru dari gambar ini, menghasilkan 32 channel fitur.
# - Conv2 menggabungkan gambar asli (64 channel) dan hasil dari Conv1 (32 channel), menghasilkan total 96 channel sebagai input untuk Conv2.
# Proses ini terus berlanjut hingga kita memiliki jumlah fitur yang cukup kaya pada akhir blok.
# Blok ini pada dasarnya mengekstraksi fitur secara bertahap dan menggabungkan informasi dari lapisan sebelumnya, 
# menghasilkan representasi yang lebih baik dari gambar sebelum menerapkan makeup.
...
##Bayangkan proses ini seperti sebuah proses di mana wajah seseorang melewati tiga tahap peningkatan detail:
# - RDB1: Pada tahap pertama, fitur dasar wajah seperti kontur mata, hidung, atau bibir mulai ditingkatkan.
# - RDB2: Pada tahap kedua, fitur ini diolah lebih lanjut, menambah detail halus, seperti bayangan atau warna kulit yang lebih akurat.
# - RDB3: Pada tahap ketiga, detail-detail yang lebih kompleks mulai diterapkan sehingga model dapat memahami lebih baik detail wajah.
# Setelah ketiga blok tersebut selesai, model menambahkan informasi asli dari wajah yang belum dimodifikasi (melalui skip connection). 
# Ini memastikan bahwa hasil akhirnya tidak terlalu jauh berubah dari gambar asli, namun tetap memiliki detail yang lebih tajam dan kaya.
...
class Generator(nn.Module):
    ...
    def __init__(self):
        ...
        layers = nn.Sequential(
            #Ini adalah layer konvolusi pertama yang mengambil input gambar berukuran (3 channel, yaitu RGB), kemudian mengubahnya menjadi 64 channel dengan ukuran kernel 7x7. 
            # Stride diatur ke 1, sehingga tidak mengurangi ukuran gambar, dan padding ditambahkan agar hasilnya tetap sama seperti input.
            nn.Conv2d(3, 64, kernel_size=7, stride=1, padding=3, bias=False),
            #menormalkan gambar di setiap channel secara independen, menjaga stabilitas pembelajaran. 
            nn.InstanceNorm2d(64, affine=True),
            #Memperkenalkan non-linearitas, yang membantu model belajar pola makeup yang lebih rumit.
            nn.ReLU(inplace=True)
        )
        #Layer-layer ini digabungkan ke dalam satu modul sekuensial dan disimpan sebagai self.pnet_in, yang akan dipanggil dalam proses forward nanti.
        self.pnet_in = layers

        # Down-Sampling 
        # Mengurangi resolusi gambar secara bertahap agar model bisa mengekstrak fitur yang lebih high-level dari gambar wajah.
        curr_dim = 64
        for i in range(2):
            layers = nn.Sequential(
    ...
    def get_weight(self, mask_c, mask_s, fea_c, fea_s, diff_c, diff_s):
        ...
        mask_c_re = F.interpolate(mask_c, size=32).repeat(1, channel_num, 1, 1)  # (3, c, h, w)
        diff_c = F.interpolate(diff_c, size=32)
        diff_s = F.interpolate(diff_s, size=32)
        fea_c = fea_c.repeat(3, 1, 1, 1)  # (3, c, h, w)
        fea_c = fea_c * mask_c_re  # (3, c, h, w) 3 stands for 3 parts

        mask_s_re = F.interpolate(mask_s, size=32).repeat(1, channel_num, 1, 1)
